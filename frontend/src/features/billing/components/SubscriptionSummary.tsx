...
import { formatStorageUsage } from '@/utils/format-storage';
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        {summary.subscription.autoRenew ? (
          <Tag color="green">Auto-renew</Tag>
        ) : (
          <Tag>Manual renewal</Tag>
        )}
      </div>
      ...
      <Descriptions column={{ xs: 1, sm: 2 }} size="small" className="mb-6">
        ...
        <Descriptions.Item label="Monthly price">
          ₹{summary.plan.monthlyPrice.toLocaleString()}
        </Descriptions.Item>
      </Descriptions>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <UsageBar
          ...
          detail={formatStorageUsage(summary.usage.storageUsedGB, summary.limits.storageLimitGB)}
        />
        <UsageBar
          label="Scans"
          percent={summary.percentages.scans}
          detail={`${summary.usage.scanUsage} / ${summary.limits.monthlyScanLimit}`}
        />
        <UsageBar
          label="Albums"
          percent={summary.percentages.albums}
          detail={`${summary.usage.albumCount} / ${summary.limits.maxAlbums}`}
        />
        <UsageBar
          label="Users"
          percent={summary.percentages.users}
          detail={`${summary.usage.userCount} / ${summary.limits.maxUsers}`}
        />
      </div>
    </div>
  );
