...
import { usePlatformAnalyticsReportsQuery } from '@/hooks/useAnalyticsQueries';
...
export const PlatformReportsPage = () => {
  ...
  const { data, isLoading } = usePlatformAnalyticsReportsQuery(params);

  const handleExport = async (format: 'csv' | 'xlsx') => {
    try {
      setExporting(true);
      const blob = await analyticsService.exportPlatformReport({ from, to, eventType, format });
      downloadAnalyticsExport(blob, `platform-analytics.${format === 'csv' ? 'csv' : 'xlsx'}`);
    } catch (error) {
    ...
    <div>
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <Paragraph type="secondary">Cross-studio analytics with export support.</Paragraph>
        </div>
